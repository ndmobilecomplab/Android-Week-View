 ...
 */
public class WeekViewPerDay extends View {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            switch (mCurrentScrollDirection) {
                ...
                case RIGHT:
                    ...
                    ViewCompat.postInvalidateOnAnimation(WeekViewPerDay.this);
                    break;
                case VERTICAL:
    ...
    public WeekViewPerDay(Context context) {
        this(context, null);
    }

    public WeekViewPerDay(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public WeekViewPerDay(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void goToNearestOrigin() {
        ...
        if (nearestOrigin != 0 && mayScrollHorizontal) {
            ...
            if (mCurrentScrollDirection == Direction.LEFT) {
                ...
                int distance = checkIfValuesAreOk((int) mOriginOfNextWeek, (int) mCurrentOrigin.x);
                ...
                float distance = checkIfValuesAreOk((int) mOriginOfPastWeek, (int) mCurrentOrigin.x);
                mScroller.startScroll((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, (int) distance, 0, (int) 200);
            }
            ViewCompat.postInvalidateOnAnimation(WeekViewPerDay.this);
        }
    ...
    private int checkIfValuesAreOk(int whereToGo, int origin) {
        int distance = whereToGo - origin;

        //It's custom start
        /*if(delta != customStart){
            delta += (customStart - mCurrentOrigin.x);
        }*/

//        if ((origin + distance) % sizeOfWeekView != customStart) {
//            origin += customStart;
//
//            float delta = (origin + distance) % sizeOfWeekView;
//            delta = delta;
//        }

        /*if (delta < 0) {
            return (int) (distance - (sizeOfWeekView + delta));
        } else if (delta > 0) {
            return (int) (distance + (sizeOfWeekView - delta));
        } else {*/
        return distance;
